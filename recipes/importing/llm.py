...
- для категории main-course укажи main_protein — из чего сделано блюдо: chicken, beef, pork или fish; если основа другая (овощи, грибы, морепродукты, баранина, индейка) или это не второе блюдо, оставь пустую строку; бульон, яйца и соусы основой не считаются;
...
- если во входных данных есть youtube_transcript (это и YouTube-источник, и статья, в которой найдено видео рецепта — related_video_url), у каждого шага укажи video_timestamp_seconds — целое значение start_seconds того фрагмента youtube_transcript, где автор начинает показывать именно это действие; не придумывай промежуточный тайм-код;
- для источника без youtube_transcript укажи video_timestamp_seconds=null;
- если статья содержит и текст, и youtube_transcript, рецепт составляй по тексту статьи, а транскрипт используй только для тайм-кодов шагов;
- source_links — ссылки из описания видео и закреплённого комментария автора вместе с контекстом; выбери среди них text_source_url — страницу с текстовой версией именно этого рецепта (рецепт на сайте или в блоге автора, статья, документ); ссылки на главные страницы сайтов, каналы, соцсети, магазины, рекламу, курсы и другие рецепты не подходят; если подходящей ссылки нет или source_links пуст, оставь пустую строку; не придумывай ссылки и не изменяй их;
...
- объект nutrition обязателен всегда, все восемь чисел в нём обязательны: это полное КБЖУ ГОТОВОГО блюда на одну порцию и на 100 г готового блюда; калории в ккал, белки, жиры и углеводы в граммах;
- считай nutrition по конкретным количествам ингредиентов и справочным значениям продуктов, а затем обязательно поправь на то, что написано в steps: масло, которое слили, отжали или дали стечь после обжарки, учитывай лишь частично (впитавшиеся 20–40 %); маринады, рассолы и воду от замачивания, которые выливают, не учитывай; кости, кожуру, шелуху и другие выбрасываемые части не считай; при варке и запекании учитывай уварку и потерю влаги, чтобы значения на 100 г относились к готовому блюду, а не к сырой массе;
- необязательные ингредиенты (optional=true) и продукты «для подачи по желанию» в nutrition не включай;
- значения на порцию и на 100 г должны быть согласованы между собой через массу готовой порции, а калории — примерно равны 4·белки + 9·жиры + 4·углеводы;
- nutrition.notes — одно-три коротких предложения на русском о ключевых допущениях расчёта: примерная масса готовой порции, что слито или выброшено, сколько масла впиталось; без пересказа рецепта;
...
Формат:
{
  "recipes": [{
    ...
    "nutrition": {
      ...
      "notes": "Порция около 280 г. Масло после пассеровки слито, учтена примерно треть. Кости из бульона не считались."
    },
    "categories": ["soup"],
    "main_protein": "",
    "cover_image_url": "https://адрес-из-source_image_urls",
    "cover_image_search_query": "goulash beef gravy",
    "text_source_url": "https://адрес-из-source_links или пустая строка",
    "ingredients": [
...
- если изменились ингредиенты, их количества, число порций или способ приготовления, пересчитай nutrition заново по тем же правилам и обнови notes; если состав и шаги не менялись, сохрани прежние значения nutrition.
"""
...
def adapt_with_ai(document: SourceDocument, custom_prompt: str = "") -> list[dict[str, Any]]:
    ...
    source_payload = {
        ...
        "source_text": document.text if document.source_type != "youtube" else "",
        "related_video_url": document.video_url,
        "youtube_transcript": list(document.transcript_segments),
        "source_links": [dict(link) for link in document.source_links],
        "source_image_urls": {
